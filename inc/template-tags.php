...
function heisenberg_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'heisenberg' ),
...
function heisenberg_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && heisenberg_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'heisenberg' ) . '</span>', $categories_list );
		}
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		...
		$title = sprintf( __( '%1$s: %2$s', 'heisenberg' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
