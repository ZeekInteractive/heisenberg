...
function heisenberg_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'heisenberg' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'heisenberg' ),
...
function heisenberg_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'heisenberg' ) ); // WPCS: XSS OK.
		if ( $categories_list && heisenberg_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'heisenberg' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'heisenberg' ), esc_html__( '1 Comment', 'heisenberg' ), esc_html__( '% Comments', 'heisenberg' ) );
		echo '</span>';
	}
